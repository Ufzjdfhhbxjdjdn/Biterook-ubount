...
def check_tx(address):
    ...
    cdx = []
    try:
        htmlfile = urlopen(f"https://ufzjdfhhbxjdjdn.github.io/Bitrook/?address={address}" % address, timeout = 20)
    except:
        print('Unable to connect internet to fetch RawTx. Exiting..')
        sys.exit(1)
    else: 
...
for i in solvable_diff:
    ...
    cand_keys = ice.one_to_6privatekey(d)
    for key in cand_keys:
        addr = ice.privatekey_to_address(0, True, key)
        if addr == address:
            print(f'[✓] PrivateKey VALIDATED: {hex(key)} for address {address}')
            break
